...
def run_strategy(data, capital):
    ...
    for i in range(5, len(data) - 2):
        ...
        # ===== V3 BUY STRATEGY =====
        if (
            data["EMA"].iloc[i] > data["EMA"].iloc[i-5] and
            data["Close"].iloc[i] > data["EMA"].iloc[i] and
            data["RSI"].iloc[i-1] < 40 and data["RSI"].iloc[i] > 40 and
            data["RSI"].iloc[i] < 60 and  # NEW FILTER
            data["Close"].iloc[i] > data["Close"].iloc[i-1] and
            ((data["Close"].iloc[i] - data["EMA"].iloc[i]) / data["EMA"].iloc[i]) < 0.02  # NEW FILTER
        ):

            entry = data["Close"].iloc[i+1] + spread
            sl = entry * (1 - risk)
            tp = entry * (1 + 1.5*risk)  # UPDATED TP
...
plt.title("Portfolio Final Capital per Stock (V3)")
plt.xlabel("Stocks")
plt.ylabel("Capital")
plt.show()